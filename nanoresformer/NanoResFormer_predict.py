...
import pandas as pd
import torch
from nanoresformer.utils import normalize_signal, determine_batch_size, pad_signal, depad_signal
from nanoresformer.utils import prepare_windows, select_device, export_image, model_mapping
import sys
...
def NanoResFormer_predict(
    ...
    Parameters:
        shard_index_path (str): Path to the Parquet index file for sharded signals.
        ...
        Model (str, optional): Model variant to use (e.g., 'middle', 'low', 'high'). Default: 'middle'.
    ...
    if not os.path.exists(shard_index_path):
        print(f"Error: Index file not found at {shard_index_path}")
...
if __name__ == "__main__":
    # Example usage with shard index instead of CSV
    import argparse
    parser = argparse.ArgumentParser(description="Run NanoResFormer inference on sharded signals")
    parser.add_argument('index_path', type=str, help='Path to Parquet index file')
    parser.add_argument('out_dir', type=str, help='Output directory')
    parser.add_argument('--OV', type=int, default=80, help='Overlap percentage')
    parser.add_argument('--Model', type=str, default='middle', choices=['low', 'middle', 'high'])
    parser.add_argument('--export_images', action='store_true')
    parser.add_argument('--device_pref', type=str, default='auto', choices=['auto', 'cpu', 'cuda'])
